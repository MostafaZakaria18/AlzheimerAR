...
exports.ingestSensorData = asyncHandler(async(req, res)=>{
    ...
    if (!data.patient || !data.dataType ||data.value === undefined){
        return res.status(400).json({success: false , message: 'Missing required data firleds.'});
    }
    ...
    res.status(201).json({success: true, message:'Data inngested successfully', alertGenrated: isAlert,
         logId: newLog._id});
});