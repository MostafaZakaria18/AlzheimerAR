...
exports.createRelative = asyncHandler(async(req, res, next)=>{
    const {patient, recognitionKey, name, relation, messageForPatient, image_data} = req.body;
    if(!image_data){
    ...
    try{
        ...
        return next(new ErrorResponse('could not process image or save relative', 500));
    }
});
...
exports.getRelativesByPatient = asyncHandler(async(req, res, next)=>{
    ...
    res.status(200).json({success:true, count: relatives.length, data: relatives});
})
