...
const sendTokenResponse = (user, statusCode, res)=>{
    ...
    const expiresInMilliseconds = parseInt(process.env.JWT_COOKIE_EXPIRE, 10) * 24 * 60 * 60 * 1000;
    const options = {
...
exports.register = asyncHandler(async(req, res, next)=>{
    ...
    if(!patient){
        return next(new ErrorResponse(`patient with id ${patientId} not found`, 404))
    }

    if(patient.caregiver){
        return next(new ErrorResponse(`patient with id ${patientId} is alreday linked to a account`,400))
    }
