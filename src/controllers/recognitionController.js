...
exports.verifyFace = asyncHandler(async(req, res, next)=>{
    ...
    if(!imageData){
        return next(new ErrorResponse('Image data is required for recogntion',
            400));
    }
    ...
    const pythonExecutablePath = 'D:\\Mostafa\'s projects\\Gradution Project\\AlzheimerAR\\.venv\\Scripts\\python.exe';
    ...
    try{
        ...
        const exitCode = await new Promise((resolve, reject)=>{
            ...
            pythonProcess.on('error', (err)=>{
                reject(new ErrorResponse(`failed to spawn python process. check path: ${err.message}`, 500));
            });
        });
        if(exitCode !== 0){
            const errorDetails = recognizedOutput || "no output captured.";
            return next(new ErrorResponse(`recgntion script failed with exit code 
                ${exitCode}. details:${errorDetails.trim()}`, 500));
        }
        ...
        try{
            ...
            return next(new ErrorResponse(`invalid json received from recogntion script`, 500));
        }
        if(recognizedResult.status ==='match'){
            ...
            if(!recognizedPerson){
                return next(new ErrorResponse(`recognized persom data not found in databse for key: ${recognitionKey}`,404));
            }
            recognizedUser = recognizedPerson;
        }else if(recognizedResult.status === 'no_match'){
            return res.status(404).json({success: false , message:'no known face recognized', recognizedUser:null});
        }else{
            return next(new ErrorResponse(`recogntion script returned as invalid status: ${recognizedResult.status}`, 500));
        }
    }catch(error){
        return next(error);
    }finally{
